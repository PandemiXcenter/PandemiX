...
# Original version from PandemiX github-repo on 2022-07-14
# Copied back from Rasmus' personal work folder with some changes, 2022-10-05
# (Previous version now called "Old_PandemiXModelFunctions.py", and this file moved to "scripts" dir)

## Change-log 
# 2022-07-27:
# * Added tRes as a property of scheme
# (Some time in Rasmus' directory)
# * Made a function call for getting a time-series of a given parameter
# 2022-10-05:
# * Added Gillispie functions, from "NewFeaturesNotebook.ipynb" (Still needs to be improved)
...
class Scheme:
    ...
    def simulate(self):
        ...
        if (len(self.Changes) == 0):
            totT = np.linspace(self.tStart,self.tEnd,self.tRes)
        ...
        # (Assuming they are in correct order. Changes should be ordered using sortChanges())
            ...
            for i in range(len(self.Changes)):
                ...
                curT = np.linspace(tInit,curChange.t,self.tRes) 
            ...
            curT = np.linspace(curChange.t,self.tEnd,self.tRes) # TODO: Flag for time-resolution?
    ...
    # Helper function for returning an array of a parameter over time, when set using "SetParameters"
    def getSetParametersOverTime(self,parameterName):

        # First period before any changes
        # Since the time-array has length tRes, it is simply an array of the initial value of length tRes
        parameterOverTime = np.ones((self.tRes,)) * self.Parameters[parameterName]

        # For each change, add "tRes" values of the set parameter over time
        for i in range(0,len(self.Changes)):            
            newParameterRange = np.ones((self.tRes,)) * self.Changes[i].SetParameters[parameterName]
            parameterOverTime = np.append(parameterOverTime,newParameterRange)

        return parameterOverTime
...
# # Start with the first period before the change
# tTest = np.linspace(0,sch2.Changes[0].t,100) 
# parameterOverTime = np.ones(tTest.shape) * sch2.Parameters['tau']

# parameterOverTime = np.ones((100,)) * sch2.Parameters['tau']

# for i in range(0,len(sch2.Changes)-1):
#     # tTest
#     newRange = np.linspace(sch2.Changes[i].t,sch2.Changes[i+1].t,100)
#     tTest = np.append(tTest,newRange)
    
#     newParameterRange = np.ones((100,)) * sch2.Changes[i].SetParameters['tau']
#     parameterOverTime = np.append(parameterOverTime,newParameterRange)

# lastRange = np.linspace(sch2.Changes[-1].t,sch2.tEnd,100)
# tTest = np.append(tTest,lastRange)


# newParameterRange = np.ones((100,)) * sch2.Changes[-1].SetParameters['tau']
# parameterOverTime = np.append(parameterOverTime,newParameterRange)
# len(sch2.result.t)


# np.all(sch2.result.t == tTest)