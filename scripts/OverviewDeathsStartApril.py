...
dfDataDeaths = pd.read_csv(latestdir+'/Deaths_over_time.csv',encoding='utf8',delimiter = ';')

dfDataDeaths = dfDataDeaths.iloc[:-1]
dfDataDeaths['Dato'] = pd.to_datetime(dfDataDeaths['Dato'])
...
def getPopulationFromCases(curStatus=sNone,curAge='Alle',weekNames=weekNames):
    ...
    curInci = getTimeSeries(df3,curStatus,curAge,weekNames)

    curPop = 100000* curCases/curInci 

    return curPop 

def getPopulationFromTests(curStatus=sNone,curAge='Alle',weekNames=weekNames):
    curCases = getTimeSeries(df2_T,curStatus,curAge,weekNames)
    # curInci = getTimeSeries(df3_C,curStatus,curAge,weekNames)
...
def getTimeSeriesAllIncidence(thisdf=df2_C,curAge='Alle',weekNames=weekNames):
    ...
    pNone = getPopulationFromTests(sNone,curAge,weekNames)
    pOne  = getPopulationFromTests(sOne,curAge,weekNames)
    pTwo  = getPopulationFromTests(sTwo,curAge,weekNames)
...
ax1.plot(dfDataDeaths.Dato,dfDataDeaths['Antal_døde'],'r.-',markersize=3,linewidth=0.5,label='Daglig data, "data"-mappe')
ax1.plot(rnTime(dfDataDeaths.Dato,7),rnMean(dfDataDeaths['Antal_døde'],7),'r',label='7-dages gennemsnit, "data"-mappe')
...
# curCases = getTimeSeriesAll(df2_C,curAge='80+')
# ax1.plot(weekDTs,curCases)

for i in range(2,len(ageGroupsGrouped)):
    curAge = ageGroupsGrouped[i]
    curName = ageGroupsGroupedNames[i]

    # curCases = getTimeSeriesAll(df2_C,curAge=curAge)
    # # ax1.plot(weekDTs,curCases,'.-',label=curName)
    # ax1.plot(weekDTs[3:],rnMean(curCases,4)*4,'.-',label=curName)

    curCases = getTimeSeriesAllIncidence(df3_C,curAge=curAge)
    # ax1.plot(weekDTs,curCases/1000,'.-',label=curName)
    ax1.plot(weekDTs[3:],rnMean(curCases/1000,4)*4,'.-',label=curName)
    curDeaths = getTimeSeriesAll(df2_D,curAge=curAge)
    ax2.plot(weekDTs,curDeaths,'.-',label=curName)
...
# %%
curAge = ageGroupsGrouped[2]
totPopSize = getPopulationFromCases(sNone,curAge) + getPopulationFromCases(sOne,curAge) + getPopulationFromCases(sTwo,curAge)
totPopSize

# %%

# fig,(ax1,ax2) = plt.subplots(2,1,sharex=True)
fig,allAxes = plt.subplots(len(ageGroupsGrouped),1,figsize=(12,20))
...
for i in range(len(ageGroupsGrouped)):
    ...
    curCases = getTimeSeriesAllIncidence(df3_C,curAge=curAge)
    # ax1.plot(weekDTs,curCases/1000,'.-',label=curName)
    ax1.plot(weekDTs[3:],rnMean(curCases,4)*4,'.-',color='xkcd:blue')

    ax1.set_title(curName)
    ax1.set_ylabel('4-week sum of cases')
    ax2.set_ylabel('Deaths')


    curDeaths = getTimeSeriesAll(df2_D,curAge=curAge)
    ax2.plot(weekDTs,curDeaths,'.--',color='xkcd:red')
    ...
    ax1.set_xlim(left=weekDTs[2])
    ax1.set_xlim(right=weekDTs[-1]+np.timedelta64(7,'D'))
...
if saveFigures:
    fig.savefig(path_figs+'AgeDistribution\\DeathsAndFourWeekCases')
